...
This file explains how to test the project manually: how to start it, what each endpoint does, what JSON to send, and how to verify restaurants, transactions, N+1, and also the `Customer`, `Booking`, and `BookingStatus` parts of the model.
...
## 6. Customer And Booking Endpoints

These endpoints let you show the other domain entities through REST, not through SQL.
...
Purpose: get one customer by id.
...
- create a booking
- attach it to existing customer
- attach it to existing restaurant table
- set initial `BookingStatus`

Before testing:

1. create a restaurant with tables
2. create a customer
3. take one real `tableId`
4. take one real `customerId`

Request JSON:

```json
{
  "bookingTime": "2026-03-10T19:00:00",
  "status": "CREATED",
  "customerId": 1,
  "tableId": 1
}
...
- response includes table info
- response includes restaurant info
...
- each booking contains `status`
- each booking is linked to customer and table

### `GET /api/v1/bookings/{id}`

Purpose: get one booking by id.
...
```bash
curl -X PUT http://localhost:8080/api/v1/bookings/1/status \
  -H "Content-Type: application/json" \
  -d '{
    "status": "CONFIRMED"
  }'
...
- status changes from `CREATED` to `CONFIRMED`
- status changes from `CONFIRMED` to `CANCELED`

### `GET /api/v1/bookings/statuses`

Purpose: show all available enum values from `BookingStatus`.

Example:

```bash
curl http://localhost:8080/api/v1/bookings/statuses
```
...
### How To Show These Entities During Demo

Recommended order:

1. create restaurant with tables
2. create customer
3. create booking with `status = CREATED`
4. open `GET /api/v1/bookings`
5. update booking to `CONFIRMED`
6. update booking to `CANCELED`
7. call `GET /api/v1/bookings/statuses`

This proves:

- `Customer` is created and returned via REST
- `Booking` links `Customer` and `RestaurantTable`
- `BookingStatus` enum is used in API and persisted correctly
...
## 9. Suggested Full Manual Test Order
...
8. Run `POST /api/v1/customers`.
9. Run `GET /api/v1/customers`.
10. Run `POST /api/v1/bookings`.
11. Run `GET /api/v1/bookings`.
12. Run `PUT /api/v1/bookings/{id}/status`.
13. Run `GET /api/v1/bookings/statuses`.
14. Run `GET /api/v1/nplusone/problem`.
15. Run `GET /api/v1/nplusone/optimized`.
16. Run `POST /api/v1/transactions/partial`.
17. Run `GET /api/v1/transactions/state`.
18. Run `POST /api/v1/transactions/rollback`.
19. Run `GET /api/v1/transactions/state`.
20. Run `POST /api/v1/transactions/exception-no-tx`.
21. Run `POST /api/v1/transactions/exception-with-tx`.

## 10. Quick Checks For Each Requirement
...
Requirement: show `Customer`, `Booking`, `BookingStatus`

- test `POST /api/v1/customers`
- test `POST /api/v1/bookings`
- test `PUT /api/v1/bookings/{id}/status`
- test `GET /api/v1/bookings/statuses`
...
- `customerId` in booking request really exists
- `tableId` in booking request really exists
